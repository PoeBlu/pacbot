...
def build_and_push_docker_image(provider_json_file, ecr_repo, docker_file, docker_file_dir, log_file):
    ...
    write_to_debug_log(
        log_file,
        f"Docker image creation and push to ecr repo: {str(ecr_repo)} is started",
    )
    ...
    latest_tag = "latest"
    if pushed := docker_client.push(
        ecr_repo, tag=latest_tag, auth_config=auth_config_payload
    ):
        write_to_debug_log(log_file, f"Pushed docker image to repo: {ecr_repo}")
    else:
        write_to_log_file(log_file, f" ERROR: failed to push. {ecr_repo}")
        raise Exception(f"Failed to push image: {str(pushed)}")
...
def delete_docker_images_from_local(docker_file_abs_path):
    ...
    write_to_debug_log(log_file, f"Deleted image {ecr_repo} from local !!!")
    ...
    try:
        ...
        write_to_debug_log(
            log_file, f"Deleted image {local_image_name} from local !!!"
        )
    except:
        pass
