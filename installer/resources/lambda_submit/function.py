...
class SubmitJobLambdaFunction(LambdaFunctionResource):
    ...
    handler = f"{BATCH_JOB_FILE_NAME}.lambda_handler"
    runtime = "python2.7"
    ...
    environment = {
        'variables': {
            ...
            'CONFIG_URL': f"{ApplicationLoadBalancer.get_api_base_url()}/config/batch,inventory/prd/latest",
    ...
    target_input = json.dumps(
        {
            ...
            'environmentVariables': [
                {
                    'name': "CONFIG_URL",
                    'value': f"{ApplicationLoadBalancer.get_api_base_url()}/config/batch,inventory/prd/latest",
                },
                {'name': "CONFIG_CREDENTIALS", 'value': "dXNlcjpwYWNtYW4="},
                {
                    'name': "CONFIG_SERVICE_URL",
                    'value': f"{ApplicationLoadBalancer.get_http_url()}/api/config/rule/prd/latest",
                },
            ],
            'params': [
                {
    ...
    target_input = json.dumps(
        {
            ...
            'environmentVariables': [
                {
                    'name': "CONFIG_URL",
                    'value': f"{ApplicationLoadBalancer.get_api_base_url()}/config/batch,data-shipper/prd/latest",
                },
                {
                    'name': "ASSET_API_URL",
                    'value': ApplicationLoadBalancer.get_api_version_url(
                        'asset'
                    ),
                },
                {
