        ...
        return [
            {
                'local-exec': {
                    ...
                    'environment': {
                        ...
                        'ENV_LOGGING_ES_HOST_NAME': ESDomain.get_output_attr(
                            'endpoint'
                        ),
                        ...
                        'ENV_ES_HEIMDALL_HOST_NAME': ESDomain.get_output_attr(
                            'endpoint'
                        ),
                        'ENV_ES_HEIMDALL_PORT': str(ESDomain.get_es_port()),
                        'ENV_ES_HEIMDALL_CLUSTER_NAME': ESDomain.get_input_attr(
                            'domain_name'
                        ),
                        ...
                        'ENV_RDS_USERNAME': MySQLDatabase.get_input_attr(
                            'username'
                        ),
                        'ENV_RDS_PASSWORD': MySQLDatabase.get_input_attr(
                            'password'
                        ),
                        'ENV_JOB_BUCKET_REGION': AwsRegion.get_output_attr('name'),
                        'ENV_RULE_JOB_BUCKET_NAME': BucketStorage.get_output_attr(
                            'bucket'
                        ),
                        'ENV_JOB_LAMBDA_REGION': AwsRegion.get_output_attr('name'),
                        'ENV_JOB_FUNCTION_NAME': SubmitJobLambdaFunction.get_input_attr(
                            'function_name'
                        ),
                        'ENV_JOB_FUNCTION_ARN': SubmitJobLambdaFunction.get_output_attr(
                            'arn'
                        ),
                        'ENV_RULE_BUCKET_REGION': AwsRegion.get_output_attr(
                            'name'
                        ),
                        'ENV_RULE_JOB_BUCKET_NAME': BucketStorage.get_output_attr(
                            'bucket'
                        ),
                        'ENV_RULE_LAMBDA_REGION': AwsRegion.get_output_attr(
                            'name'
                        ),
                        'ENV_RULE_FUNCTION_NAME': RuleEngineLambdaFunction.get_input_attr(
                            'function_name'
                        ),
                        'ENV_RULE_FUNCTION_ARN': RuleEngineLambdaFunction.get_output_attr(
                            'arn'
                        ),
                        ...
                        'ENV_BASE_ACCOUNT': AwsAccount.get_output_attr(
                            'account_id'
                        ),
                        ...
                        'ENV_CONFIG_SERVICE_URL': f"{ApplicationLoadBalancer.get_http_url()}/api/config/rule/prd/latest",
                        'ENV_PACBOT_AUTOFIX_RESOURCEOWNER_FALLBACK_MAILID': Settings.get(
                            'USER_EMAIL_ID', ""
                        ),
                    },
...
class ImportDbSql(NullResource):
    ...
    def get_provisioners(self):
        ...
        return [
            {
                'local-exec': {
                    'command': f"mysql -u {db_user_name} -p{db_password} -h {db_host} < {ReplaceSQLPlaceHolder.dest_file}"
                }
            }
        ]
